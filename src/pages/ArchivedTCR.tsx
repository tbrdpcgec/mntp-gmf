...
const COLUMN_ORDER: { key: string; label: string }[] = [
  ...
  { key: 'status_sm1', label: 'TCR-1' },
  { key: 'remark_sm1', label: 'Remark TCR-1' },
  { key: 'handle_by_sm1', label: 'Handle by TCR-1' },
  { key: 'date_closed_sm1', label: 'Date Closed TCR-1' },

  { key: 'status_cs1', label: 'TCR-2' },
  { key: 'remark_cs1', label: 'Remark TCR-2' },
  { key: 'handle_by_cs1', label: 'Handle by TCR-2' },
  { key: 'date_closed_cs1', label: 'Date Closed TCR-2' },

  { key: 'status_sm4', label: 'TCR-3' },
  { key: 'remark_sm4', label: 'Remark TCR-3' },
  { key: 'handle_by_sm4', label: 'Handle by TCR-3' },
  { key: 'date_closed_sm4', label: 'Date Closed TCR-3' },

  { key: 'status_cs4', label: 'TCR-4' },
  { key: 'remark_cs4', label: 'Remark TCR-4' },
  { key: 'handle_by_cs4', label: 'Handle by TCR-4' },
  { key: 'date_closed_cs4', label: 'Date Closed TCR-4' },

  { key: 'status_mw', label: 'TCR-5' },
  { key: 'remark_mw', label: 'Remark TCR-5' },
  { key: 'handle_by_mw', label: 'Handle by TCR-5' },
  { key: 'date_closed_mw', label: 'Date Closed TCR-5' },
...
export default function BUSH4() {
  ...
  const handleAction = async (action: 'copy' | 'exportAndDelete') => {
    ...
    switch (action) {
      ...
      case 'exportAndDelete':
        ...
        XLSX.writeFile(wb, `Mntp_archived_${new Date().toISOString()}.xlsx`);
  ...
  const filteredRows = rows
    .filter((row) => {
      ...
      return matchesSearch && matchesAcReg && matchesDocStatus;
    })
    .sort((a, b) => {
      if (!sortKey) return 0;
