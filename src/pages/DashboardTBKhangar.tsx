...
import { supabase, supabaseSecond } from '../supabaseClient';
...
const docStatusList = ['🔴NEED RO', '🟡RO DONE', '🟢COMPLETED', '🔘CANCEL'];
...
const gridCols =
  'grid-cols-[70px_90px_90px_28px_28px_28px_28px_80px_60px_28px_28px_28px_38px_90px]';
...
export default function BUSH4() {
  ...
  const [statusDocFilter, setStatusDocFilter] = useState('ALL');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      while (moreData) {
        const { data, error } = await supabaseSecond
          .from('mdr_tracking_tbk1')
          .select('*')
          .eq('archived', false)
      ...
      // Ambil data ABMP dari Supabase utama
      const { data: abmpData, error: abmpError } = await supabase
        .from('abmp')
        .select('ac_reg,rts,customer,line');

      if (abmpError) {
        console.error('Error ABMP:', abmpError);
      }

      // Buat map agar lookup cepat
      const abmpMap = new Map(
        (abmpData || []).map((a) => [a.ac_reg?.trim().toUpperCase(), a])
      );

      // Gabungkan MDR + ABMP
      const mergedRows = allRows.map((row) => {
        const abmp = abmpMap.get(row.ac_reg?.trim().toUpperCase());

        return {
          ...row,

          // override / tambahkan dari ABMP
          rts: abmp?.rts ?? row.rts,
          customer: abmp?.customer ?? row.customer,
          line: abmp?.line ?? row.line,
        };
      });
  ...
  const filteredRowsDetail = useMemo(() => {
    ...
    // 2️⃣ Filter dari kotak DOC STATUS
    if (docStatusFilter === 'NEED_RO') {
      rows = rows.filter((r) =>
        String(r.doc_status || '')
          .toUpperCase()
          .includes('NEED RO')
    ...
    return rows;
  }, [filteredRows, selectedSummaryAcRegs, filterStatusJob, docStatusFilter]);
  ...
  const projectSummary = useMemo(() => {
    ...
    return uniqAcReg.map((acReg, index) => {
      ...
      // ================= DOC STATUS SUMMARY =================
      const needRo = rowsByAc.filter((r) =>
        String(r.doc_status || '').includes('NEED RO')
      ).length;

      const roDone = rowsByAc.filter((r) =>
        String(r.doc_status || '').includes('RO DONE')
      ).length;

      const completed = rowsByAc.filter((r) =>
        String(r.doc_status || '').includes('COMPLETED')
      ).length;

      const cancel = rowsByAc.filter((r) =>
        String(r.doc_status || '').includes('CANCEL')
      ).length;
  ...
  const docStatusSummary = useMemo(() => {
    return {
      needRo: filteredRows.filter((r) =>
        String(r.doc_status || '')
          .toUpperCase()
          .includes('NEED RO')
      ).length,

      roDone: filteredRows.filter((r) =>
        String(r.doc_status || '')
          .toUpperCase()
          .includes('RO DONE')
      ).length,

      completed: filteredRows.filter((r) =>
        String(r.doc_status || '')
          .toUpperCase()
          .includes('COMPLETED')
      ).length,

      cancel: filteredRows.filter((r) =>
        String(r.doc_status || '')
          .toUpperCase()
          .includes('CANCEL')
      ).length,
  ...
  ////helper filter sttuas doc
  const handleDocStatusFilter = (filterValue: string, docStatus: string) => {
    const next = docStatusFilter === filterValue ? 'ALL' : filterValue;

    setDocStatusFilter(next);
    ...
    const acRegs = projectSummary
      .filter((r) => {
        ...
        return r.doc_status === docStatus && remain !== null && remain >= 0;
      })
      .map((r) => r.acReg);
  ...
  const handleDocStatusClick = (filterKey: string, statusText: string) => {
    const next = docStatusFilter === filterKey ? '' : filterKey;
      ...
      ...new Set(
        filteredRows
          .filter((r) =>
            String(r.doc_status || '')
              .toUpperCase()
              .includes(statusText.toUpperCase())
          )
          .map((r) => r.ac_reg)
      ),
    ];
  ...
  console.log('docStatusFilter:', docStatusFilter);
  ...
  const filteredProjectSummary = projectSummary.filter((row) => {
    ...
    if (customerFilter === 'GARUDA' && c !== 'GARUDA INDONESIA') {
      return false;
    }

    if (customerFilter === 'CITILINK' && c !== 'CITILINK') {
      return false;
    }
    ...
    if (customerFilter === 'RTS7') {
      ...
      if (!(remain !== null && remain >= 0 && remain <= 7)) {
        return false;
      }
    }

    // ================= DOC STATUS =================
    if (docStatusFilter === 'NEED_RO' && row.needRo === 0) {
      return false;
    }

    if (docStatusFilter === 'RO_DONE' && row.roDone === 0) {
      return false;
    }

    if (docStatusFilter === 'COMPLETED' && row.completed === 0) {
      return false;
    }

    if (docStatusFilter === 'CANCEL' && row.cancel === 0) {
      return false;
    }
  ...
  const ProcessChart = ({ title, chart }: any) => {
    ...
    const data = [
      ...
      {
        name: 'PROGRESS',
        value: chart.progress || 0,
        color: '#eab308',
      },
      {
        name: 'CLOSED',
        value: chart.closed || 0,
        color: '#22c55e',
      },
      {
        name: 'UNDEFINED',
        value: chart.undefinedStatus || 0,
        color: '#3b82f6',
      },
    ];

    return (
      <div className="border rounded-[10px] shadow bg-white p-2 w-[225px]">
        {/* TITLE */}
        <h3 className="text-[11px] font-bold text-center text-gray-700 mb-1">
          {title}
        </h3>
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="flex flex-col gap-1 text-[10px]">
            <div className="flex items-center gap-2">
              ...
              <span>{chart.undefinedStatus || 0}</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        {/* FOOTER */}
        <div className="border-t mt-2 pt-1 text-[11px] font-bold text-gray-700">
          TOTAL : {chart.total || 0}
        </div>
      </div>
    );
      ...
      <div className="bg-[#141414] w-full h-full">
        ...
        `}
          ...
          <div className="w-full flex gap-4 items-start">
            {/* ================= LEFT: CHART ================= */}
            <div className="w-full min-w-[390px] max-w-[33vw] flex flex-col gap-2 shrink-0">
...
`}
                >
                  <div className="text-sm mb-3">GARUDA</div>
...
`}
                >
                  <div className="text-sm mb-3">CITILINK</div>
                  <div className="text-2xl font-bold">
...
`}
                >
                  <div className="text-sm mb-3">NON</div>
                  <div className="text-2xl font-bold">
  ...
  `}
                >
                  <div className="text-sm mb-3">RTS D-7</div>
                  <div className="text-2xl font-bold">
              ...
              <div className="grid grid-cols-4 gap-4 flex-wrap">
                ...
                <div
                  onClick={() => {
                    const next = docStatusFilter === 'NEED_RO' ? '' : 'NEED_RO';
                    setDocStatusFilter(next);
                    if (next === '') {
                      setSelectedSummaryAcRegs([]);
                      return;
          ...
          : 'bg-white border-red-200'
                ...
                >
                  ...
                  <div className="text-2xl font-bold text-red-600">
                    {docStatusSummary.needRo}
                  </div>
                </div>

                {/* RO DONE */}
                <div
                  onClick={() => handleDocStatusClick('RO_DONE', 'RO DONE')}
                  className={`cursor-pointer min-w-[110px]
      rounded-xl px-3 py-2 border transition-all
      hover:scale-105
      ${
        docStatusFilter === 'RO_DONE'
          ? 'bg-yellow-100 border-yellow-500 shadow-md'
          : 'bg-white border-yellow-200'
      }`}
                >
                  <div className="text-[11px] text-yellow-600 font-semibold">
                    🟡 RO DONE
                  </div>
                  <div className="text-2xl font-bold text-yellow-600">
                    {docStatusSummary.roDone}
                  </div>
                </div>

                {/* COMPLETED */}
                <div
                  onClick={() => handleDocStatusClick('COMPLETED', 'COMPLETED')}
                  className={`cursor-pointer min-w-[110px]
      rounded-xl px-3 py-2 border transition-all
      hover:scale-105
      ${
        docStatusFilter === 'COMPLETED'
          ...
          : 'bg-white border-green-200'
                ...
                >
                  ...
                  <div className="text-2xl font-bold text-green-600">
                    {docStatusSummary.completed}
                  </div>
                </div>

                {/* CANCEL */}
                <div
                  onClick={() => handleDocStatusClick('CANCEL', 'CANCEL')}
                  className={`cursor-pointer min-w-[110px]
      rounded-xl px-3 py-2 border transition-all
      hover:scale-105
      ${
        docStatusFilter === 'CANCEL'
  ...
  `}
                    >
                      ...
                      <div className="grid-cell">LINE</div>

                      {/* LOCATION */}
                      <div className="rotate-header bg-teal-600">NRO</div>
                      <div className="rotate-header bg-teal-600">RDN</div>
                      <div className="rotate-header bg-teal-600">CMP</div>
                      <div className="rotate-header bg-teal-600">CNC</div>
      ...
      `}
                        >
                          ...
                          <div className="grid-cell text-gray-600 font-bold">
                            {row.cancel}
                          </div>
                          {/* PLAN RTS */}
                          <div className="grid-cell text-gray-800 font-bold">
      ...
      `}
                >
                  <tr className="sticky  z-10 h-7 bg-[#00838f] text-white text-xs font-semibold text-center">
                    ...
                    <th className="px-1 py-1 min-w-[50px]">PRIORITY</th>
                    ...
                    >
                      ...
                      <td className="border px-2 py-1 text-left align-top">
                        <div className="flex flex-col gap-0.5">
                          {/* BARIS 1 */}
                          {(() => {
                            const idLine = [row.order].filter(Boolean);

                            return (
                              idLine.length > 0 && (
                                <span className="font-bold  text-[13px]  text-blue-600">
                                  {idLine.join(' || ')}
                                </span>
                              )
                            );
      ...
      ${
        row.status_job === 'OPEN'
          ...
          : 'bg-gray-400'
                      ...
                      <td className="border px-1 py-1 text-left break-words w-[100px]">
                      ...
                      <td className="border px-1 py-1 text-left break-words w-[120px]">
