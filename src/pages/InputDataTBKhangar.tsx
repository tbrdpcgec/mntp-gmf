...
const getStatusPE = (doc_status: string): string => {
  const progressStatus = ['🟡RO DONE'];
  const closedStatus = ['🟢COMPLETED'];

  if (['🔴NEED RO', '🟡RO DONE'].includes(doc_status)) return 'OPEN';
...
export default function InputDataTBKhangar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data, error } = await supabaseSecond.from('mdr_tracking_tbk1')
        .select(`
          id,
          order,
          description,
  ...
  const handleToggle = async (row: any, key: string, value: string) => {
    ...
    const { error } = await supabaseSecond
      .from('mdr_tracking_tbk1') // ✅ FIX DI SINI
      .update({ [key]: value })
      .eq('id', row.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (checkError) {
      console.error('Failed to check for duplicate orders:', checkError);
      return;
    }
    ...
    if (finalDataToInsert.length === 0) {
      setMessage(
        '❌ All orders already exist in the database. Nothing was saved.'
      );
      setLoading(false);
      return;
    }

    const { error: insertError } = await supabaseSecond
      .from('mdr_tracking_tbk1')
    ...
    if (insertError) {
      ...
      setMessage(
        `✅ ${finalDataToInsert.length} Data saved successfully.${
          duplicatedData.length > 0
            ? ` ${duplicatedData.length} Duplicate orders were not saved.`
            : ''
        }`
      );
  ...
  const handleUpdate = async (id: string, payload: Record<string, any>) => {
    await supabaseSecond.from('mdr_tracking_tbk1').update(payload).eq('id', id);
  };
