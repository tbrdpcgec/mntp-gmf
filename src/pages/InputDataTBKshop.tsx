...
export default function InputDataTBKshop() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data, error } = await supabaseSecond.from('mdr_tracking_tbk')
        .select(`
          id,
          order,
          description,
  ...
  const handleToggle = async (row: any, key: string, value: string) => {
    ...
    const { error } = await supabaseSecond
      .from('mdr_tracking_tbk') // ✅ FIX DI SINI
      .update({ [key]: value })
      .eq('id', row.id);
  ...
  const handleUpdate = async (id: string, payload: Record<string, any>) => {
    await supabaseSecond.from('mdr_tracking_tbk').update(payload).eq('id', id);
  };
