...
export default function InputDataTBKshop() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data, error } = await supabaseSecond.from('mdr_tracking').select(`
          id,
          order,
          description,
          ac_reg,
  ...
  const handleUpdate = async (id: string, payload: Record<string, any>) => {
    await supabaseSecond.from('mdr_tracking').update(payload).eq('id', id);
  };
