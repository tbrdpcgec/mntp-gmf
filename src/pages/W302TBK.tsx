...
function DownloadPDFButton({ pdfData, shift, filterReportOnly }) {
  ...
  const handleConfirm = async () => {
    ...
    const doc = <W302PDF data={pdfData} />;
    ...
    link.href = url;
    link.download = `E-HOB MANTAP COMPOSITE TBK ${pdfData.date} ${shift}.pdf`;
  ...
  return (
    <div className="relative">
      <button
        onClick={() => {
          if (!filterReportOnly) {
            alert('Aktifkan Togle Report (ON) terlebih dahulu.');
            return;
          }

          setShowConfirm(true);
        }}
        className={`text-white text-[11px] px-2 py-1 rounded shadow ${
          filterReportOnly
            ? 'bg-[#d54336] hover:bg-[#aa0e00]'
...
const supervisorList = [
  ...
  '581236 / SEPTYAN SIGIT PURNOMO',
  '583312 / ADHI PUTRA AGUS SETIAWAN',
  '583317 / FAUZHY MUHAMMAD ZULKARNAIN',
  '583835 / ILHAM NUR ROSYID',
  '583941 / EL-VIAN BEZALEEL INDEPENDA LBN. TORUAN',
  '583942 / MARGO KARUNIA PUTRA',
  '584444 / BERKATITO DAVID MARCHIANO',
];
...
const managerOptions = ['581822 / JEFRI YURISTIANTO'];
...
export default function W301() {
  ...
  const [showArchivedCS1, setShowArchivedCS1] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 🔴 archive_cs1 filter pakai toggle
      if (!showArchivedCS1) {
    ...
    fetchData();
  }, [filterReportOnly, showArchivedCS1]);
        ...
        <div className=" mb-2 flex flex-wrap gap-1 items-center">
          ...
          <div className="flex flex-wrap items-center gap-1 text-xs mt-1 mb-1">
            ...
            <div className="flex flex-wrap items-center gap-1 text-xs">
              <button
              title="Daily Report Whatsapp and HOB"
              ...
              <button
                onClick={() => setShowArchivedCS1((prev) => !prev)}
  ...
  `}
              >
                {showArchivedCS1 ? 'Hide Archived' : 'Show Archived'}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-[#1b1b1b] border border-gray-700 rounded-xl p-4 w-[95%] max-w-[950px] text-white shadow-2xl">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              {/* PDF */}
              <DownloadPDFButton
              title="HOB REPORT (Turn ON the toggle before Report)"
                pdfData={pdfData}
                shift={shift}
                filterReportOnly={filterReportOnly}
              />
            </div>
          </div>
        </div>
